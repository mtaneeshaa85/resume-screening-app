...
const EduBridgeFullApp = () => {
  ...
  const parseCSV = (text) => {
    ...
    if (candidates.length === 0) {
      throw new Error(
        "No valid candidate data found. Please check your CSV format."
      );
    }
  ...
  const detectGaps = (candidate, exceptions) => {
    ...
    if (freelanceCount >= 2) {
      gaps.push(
        "Multiple freelance/consulting periods detected - verify employment continuity"
      );
      confidence += 20;
    }
    ...
    if (education.includes("graduated") || education.includes("degree")) {
      ...
      if (eduYears && expYears) {
        ...
        if (transitionGap > 1) {
          gaps.push(
            `${transitionGap} year gap between graduation (${lastEduYear}) and first employment (${firstExpYear})`
          );
    ...
    if (
      ...
      !fullText.includes(new Date().getFullYear().toString())
    ) {
      ...
      if (allYears) {
        ...
        if (currentYear - latestYear >= 1) {
          gaps.push(
            `No current employment listed - last mentioned year is ${latestYear}`
          );
          confidence += 20;
        }
      }
    ...
    const filteredGaps = gaps.filter((gap) => {
      ...
      return !exceptionLower.some((exc) => {
        ...
        if (
          ...
          return true;
        if (exc.includes("educational") && gapLower.includes("graduation"))
          return true;
        if (exc.includes("health") && gapLower.includes("health")) return true;
        if (exc.includes("family") && gapLower.includes("family")) return true;
        if (exc.includes("sabbatical") && gapLower.includes("sabbatical"))
          return true;
        if (exc.includes("military") && gapLower.includes("military"))
          return true;
        return false;
      });
    });
    ...
    return {
      ...
      analysis: `Analyzed using pattern matching, timeline analysis, and exception filtering. ${
        filteredGaps.length > 0
          ? `${filteredGaps.length} issue(s) identified.`
                ...
                {[
                  {
                    num: "1",
                    title: "Upload Data",
                    desc: "CSV or Excel file with candidate information",
                  },
                  {
                    num: "2",
                    title: "AI Analysis",
                    desc: "Automatic gap detection with confidence scoring",
                  },
                  {
                    num: "3",
                    title: "Define Exceptions",
                    desc: "Specify acceptable gaps (pre-loaded defaults available)",
                  },
                  {
                    num: "4",
                    title: "Human Review",
                    desc: "Approve or reject each flagged candidate",
                  },
                  {
                    num: "5",
                    title: "Download Results",
                    desc: "Export updated data with decisions in same format",
                  },
                ].map((step) => (
                  <li
                    key={step.num}
    ...
    const handleFileUpload = async (e) => {
      ...
      try {
        ...
        if (fileType === "csv") {
          ...
          throw new Error(
            "Unsupported file type. Please upload CSV or Excel (.xlsx, .xls) file."
          );
        }

        if (candidates.length === 0) {
          throw new Error(
            "No valid candidate data found in file. Please check the format."
          );
        }
      ...
      >
        ...
        >
          ...
          >
            <Upload
              size={64}
              style={{ margin: "0 auto 1rem", color: "#4f46e5" }}
            />
            <h3
              style={{
                fontWeight: "600",
            ...
            <p style={{ color: "#4b5563", marginBottom: "1.5rem" }}>
              ...
              <strong>name, email, education, experience</strong>
            </p>
            ...
            >
              ...
              <button
                ...
                onMouseEnter={(e) =>
                  !loading && (e.target.backgroundColor = "#15803d")
                }
...
Jane Smith,jane@email.com,MBA Harvard (2020-2022),Consultant McKinsey (2022-2024) Currently unemployed`}
      ...
      >
        ...
        >
          ...
          <p style={{ color: "#4b5563", marginBottom: "1.5rem" }}>
            ...
            {sessionData.candidates.length} candidates found
          </p>
          ...
          >
            <p style={{ color: "#166534" }}>
              ✓ {sessionData.candidates.length} candidates loaded and ready for
              AI analysis
            </p>
          </div>
          ...
          >
            ...
            >
              ...
              >
                <tr>
                  {["#", "Name", "Email", "Education", "Experience"].map(
                    (header) => (
                      <th
                        key={header}
                        style={{
      ...
      >
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "flex-start" }}>
              <Brain
                style={{ color: "#9333ea", marginRight: "1rem", flexShrink: 0 }}
                size={40}
              />
              <div>
                <h3
          ...
          >
            <p style={{ fontSize: "0.875rem", color: "#374151" }}>
              <strong>Current Exceptions:</strong>{" "}
              {sessionData.exceptions.length} defined exceptions will be
          ...
          <button
            ...
            onMouseEnter={(e) =>
              !loading && (e.target.backgroundColor = "#4338ca")
            }
            onMouseLeave={(e) =>
              !loading && (e.target.backgroundColor = "#4f46e5")
            }
          >
            {loading ? (
              <>
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p style={{ color: "#4b5563", marginBottom: "2rem" }}>
            Issues detected in {candidatesWithGaps.length} out of{" "}
            ...
            {[
              {
                ...
                label: "Gaps Detected",
                value: candidatesWithGaps.length,
                color: "#fee2e2",
                iconColor: "#dc2626",
              },
              {
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      ...
                      >
                        <AlertTriangle
                          style={{ color: "#dc2626", marginRight: "0.25rem" }}
                          size={16}
                        />
                        <span
                          style={{
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p style={{ color: "#4b5563", marginBottom: "2rem" }}>
            Review and customize acceptable gaps. These will guide your
            decisions.
          </p>
          ...
          <div style={{ marginBottom: "2rem" }}>
            ...
            >
              {localExceptions.map((exception, idx) => (
                ...
                >
                  <span style={{ color: "#374151", flex: 1 }}>{exception}</span>
                  <button
  ...
  const HumanVerificationPage = () => {
    ...
    const progressPercent =
      candidatesWithGaps.length > 0
        ? (Object.keys(decisions).length / candidatesWithGaps.length) * 100
        : 100;
      ...
      >
        ...
        >
          <div style={{ marginBottom: "2rem" }}>
            ...
            <p style={{ fontSize: "0.875rem", color: "#4b5563" }}>
              {Object.keys(decisions).length} of {candidatesWithGaps.length}{" "}
              reviewed ({cleanCandidates.length} auto-approved)
            </p>
          </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Download a CSV file with all candidates, their status, AI
